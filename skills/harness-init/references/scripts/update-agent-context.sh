...
# Template file
TEMPLATE_FILE="$REPO_ROOT/knowledge/.scripts/templates/agent-file-template.md"
...
update_existing_agent_file() {
    ...
    if [[ -n "$NEW_DB" ]] && [[ "$NEW_DB" != "N/A" ]] && [[ "$NEW_DB" != "NEEDS CLARIFICATION" ]] && [[ "$NEW_DB" != "none" ]] && ! grep -q "$NEW_DB" "$target_file"; then
    ...
    # Post-loop check: still in Active Technologies at EOF
    if [[ $in_tech_section == true ]] && [[ $tech_entries_added == false ]] && [[ ${#new_tech_entries[@]} -gt 0 ]]; then
        printf '%s\n' "${new_tech_entries[@]}" >> "$temp_file"
        tech_entries_added=true
    fi

    # If Active Technologies section doesn't exist, add it at the end
